#!/usr/bin/env python3
"""
ENS DAO Process Mining — Script 3: Performance Mining
...
OUT_PREFIX = f"{FIGURES_DIR}/ens_out_03_"
...
for cid, g in df.groupby('case:concept:name'):
    ...
    votes = g[g['concept:name'].isin(['vote', 'VoteCast'])].sort_values('time:timestamp')
    ...
    # Vote arrival distribution within voting window
    ...
    if len(votes) > 0:
        ...
        if len(vp_sorted) > 0:
            ...
            if total_vp > 0 and pd.notna(ts_voting_start):
                ...
                if pd.notna(ts_voting_end):
                    m['pct_window_to_50pct_vp'] = t50 / m.get('voting_window_h', np.nan)
...
for src in ['snapshot', 'onchain']:
    ...
    if sub.empty:
        continue
    print(f"\n  {src.upper()} (n={len(sub)}):")
    for col in ['voting_window_h', 'voting_start_to_first_vote_h', 'n_votes']:
        s = sub[col].dropna()
        if s.empty:
            continue
...
# ── 7. Dotted chart ──────────────────────────────────────────────────────
try:
    pm4py.save_vis_dotted_chart(df, f"{OUT_PREFIX}dotted_chart.png")
    print(f"\nSaved {OUT_PREFIX}dotted_chart.png")
except Exception as e:
    print(f"\nDotted chart skipped: {e}")

# ── 8. Case duration distribution ────────────────────────────────────────
try:
    pm4py.save_vis_case_duration_graph(df, f"{OUT_PREFIX}case_duration.png")
    print(f"Saved {OUT_PREFIX}case_duration.png")
except Exception as e:
    print(f"Case duration graph skipped: {e}")

# ── 9. Events distribution over time ─────────────────────────────────────
try:
    pm4py.save_vis_events_distribution_graph(df, f"{OUT_PREFIX}events_over_time.png")
