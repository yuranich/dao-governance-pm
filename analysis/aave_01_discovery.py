#!/usr/bin/env python3
"""
AAVE DAO Process Mining — Script 1: Process Discovery & Overview

Discovers process models (DFG, Petri net, BPMN, heuristic net) and
provides dataset statistics. Uses a sampled vote subset for tractable
model discovery while computing statistics on the full dataset.
"""
...
OUT_PREFIX = f"{FIGURES_DIR}/aave_out_01_"
...
print("=" * 72)
print("SCRIPT 1 — PROCESS DISCOVERY & OVERVIEW (AAVE)")
print("=" * 72)

n_events = len(df)
n_cases = df['case:concept:name'].nunique()
vote_mask = df['concept:name'].isin(['vote', 'VoteEmitted'])
n_voters = df[vote_mask]['org:resource'].nunique()
...
for src, cnt in df['source'].value_counts().items():
    print(f"  {src:<12s} {cnt:>10,}")

# Build a skeleton log: non-vote events + sampled votes (max 50 per case)
print("\nBuilding skeleton log for model discovery...")
...
print(f"Skeleton log: {len(df_skeleton):,} events ({len(df_nonvote):,} non-vote + {len(df_vote_sampled):,} sampled votes)")
...
for subset_name in ['snapshot', 'onchain']:
    df_sub = df_skeleton[df_skeleton['source'] == subset_name]
    if df_sub.empty:
        continue
...
if linked:
    df_linked = df_skeleton[df_skeleton['case:concept:name'].isin(linked)]
